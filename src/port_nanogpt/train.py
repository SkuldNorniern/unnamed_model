...
def train(file_path):
    ...
    block_size = 1024
    # model n_layer=24, n_head=16, n_embd=1024
    n_layer = 41
    n_head = 21
    n_embd = 1449
    ...
    def get_batch(split):
        ...
        x = torch.clamp(x, 0, model.config.vocab_size - 1)
        y = torch.clamp(y, 0, model.config.vocab_size - 1)
    ...
    pbar = tqdm(range(max_iters), desc="Training", disable=not master_process)
